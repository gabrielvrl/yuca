...
function HireServices({ history }) {
    ...
    useEffect(() => {
        async function loadServices() {
            ...
            const serializedUserServices = promise.data.map((service) => ({
                ...service,
                checked: false,
            }));
    ...
    useEffect(() => {
        ...
        if (sessionStorage.length !== 0) {
            const sessionStorageOptions = servicesOptions.map((service) => {
                service.checked = JSON.parse(
                    sessionStorage.getItem(`isChecked${service.id}`)
                );
                return service;
            });
